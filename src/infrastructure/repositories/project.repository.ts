...
function mapItem(item: any): ProjectItem {
  return {
    ...
    itemDepth: item.itemDepth != null ? Number(item.itemDepth) : null,
    metersPerUnit: item.metersPerUnit != null ? Number(item.metersPerUnit) : null,
    ...
    return prisma.$transaction(async (tx) => {
      const item = await tx.projectItem.create({
        data: {
          ...
          itemDepth: data.itemDepth ?? null,
          metersPerUnit: data.metersPerUnit ?? null,
          bedType: data.bedType ?? null,
          trackControl: data.trackControl ?? null,
  ...
  async updateItem(itemId: string, data: UpdateProjectItemInput): Promise<ProjectItem> {
    return prisma.$transaction(async (tx) => {
      ...
      const updated = await tx.projectItem.update({
        where: { id: itemId },
        data: {
          ...(data.itemCategory !== undefined && { itemCategory: data.itemCategory }),
          ...
          ...(data.itemDepth !== undefined && { itemDepth: data.itemDepth }),
          ...(data.metersPerUnit !== undefined && { metersPerUnit: data.metersPerUnit }),
          ...(data.bedType !== undefined && { bedType: data.bedType || null }),
          ...(data.trackControl !== undefined && { trackControl: data.trackControl || null }),
